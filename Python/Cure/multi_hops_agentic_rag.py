...
# Maximum characters of RAG context sent to LLM to prevent token exhaustion.
_MAX_CONTEXT_CHARS = 40_000
# Maximum characters allowed for the input question/prompt.
# Set high enough to fit a full synthesis prompt passed from overall_cure_synthesizer.
_MAX_QUESTION_CHARS = 32_000
# Maximum deduplicated evidence docs retained across iterations.
_MAX_EVIDENCE_DOCS = 50
...
class EnhancedRAGPipeline:
    ...
    def _analyze_question(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        prompt = ChatPromptTemplate.from_template(prompt_text)
        payload = {"question": question}  # example
    ...
    def _parallel_retrieve(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        def retrieve_with_strategy(query: str, strategy: str) -> List[Document]:
            try:
                ...
                logger.exception("Retrieval error strategy=%s", strategy)
                return []
            ...
            if state.get("sub_questions"):
                for sq in state.get("sub_questions", []):
                    if sq.get("query") == query:
                        ...
                        break
            logger.debug("Running retrieval strategy=%s", strategy)
            docs = retrieve_with_strategy(query, strategy)
            logger.debug("Retrieval returned %d docs strategy=%s", len(docs), strategy)
            parallel_results[f"{query}_{strategy}"] = docs
            all_docs.extend(docs)
        fused_docs = _dedupe_docs(all_docs)
        existing_evidence = state.get("evidence_docs", [])
        updated_evidence = _dedupe_docs(existing_evidence + fused_docs)[:_MAX_EVIDENCE_DOCS]
        new_hints = []
        for d in fused_docs:
            src = d.metadata.get("source") or d.metadata.get("file_path") or "unknown"
            page = d.metadata.get("page", "?")
    ...
    def _advanced_assess(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        raw_context = _format_docs(evidence_docs)
        context = "<evidence>\n" + raw_context[:_MAX_CONTEXT_CHARS] + "\n</evidence>"
        prompt_id,raw = self.prompt_store.get_with_id(
            module_name=self.prompt_module,
            analysis_mode=self.prompt_mode,
        ...
        logger.debug("Loaded prompt CONTEXT_ASSESSMENT_PROMPT id=%s", prompt_id)
        # 2) SAFE sanitize (avoid JSON brace errors, keep placeholders)
        if "{{" in raw or "}}" in raw:
            prompt_text = raw
        else:
            prompt_text = raw.replace("{", "{{").replace("}", "}}")
    ...
    def _intelligent_decide(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        logger.debug("Loaded prompt DECISION_MAKING_PROMPT id=%s", prompt_id)
        if "{{" in raw or "}}" in raw:
            prompt_text = raw
        else:
            prompt_text = raw.replace("{", "{{").replace("}", "}}")
    ...
    def _enhanced_synthesis(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        # Format the context with source information (capped to prevent token exhaustion)
        raw_context = _format_docs(evidence_docs)
        context = "<evidence>\n" + raw_context[:_MAX_CONTEXT_CHARS] + "\n</evidence>"
        ...
        prompt_id,raw = self.prompt_store.get_with_id(
            module_name=self.prompt_module,
            analysis_mode=self.prompt_mode,
            prompt_key=" ENHANCED_SYNTHESIS_PROMPT",
            instrument_type="-",
            lifecycle_stage="-",
        )
        logger.debug("Loaded prompt ENHANCED_SYNTHESIS_PROMPT id=%s", prompt_id)


        # Pass source documents to the synthesis prompt
        # answer = chain.invoke(
        #     {
        #         "question": question,
        #         "context": context,
    ...
    def _advanced_verify(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        logger.debug("Loaded prompt ADVANCED_VERIFICATION_PROMPT id=%s", prompt_id)
    ...
    def ask(
        ...
        logger.info("ask() called length=%d", len(question))
        ...
        if recursion_limit is not None:
            config["recursion_limit"] = max(1, min(int(recursion_limit), 200))
        final_state = self.app.invoke(init, config=config)
        answer = final_state.get("final_answer", "")
        logger.info("ask() complete grounded_ok=%s", final_state.get("grounded_ok"))
        debug = {
