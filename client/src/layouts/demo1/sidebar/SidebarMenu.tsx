...
import { MenuAuthContext } from '@/auth/AuthContext';
type RolePermission = {
  AssetName: string;
  Description?: string;
  Id: number;
...
interface SidebarMenuProps {
  menuConfig: TMenuConfig; // Define the type of menuConfig prop
}
...
const logAccess = async (label?: string, path?: string) => {
  try {
    ...
    await fetch('/api/framework/log-access', {
      ...
      body: JSON.stringify({
        ...
        Status: 'Success',
        IPAddress: '',
        UniverseID: currentUser?.UniverseID,
        Details: `Opened ${label || path} page`
      })
    });
  } catch (error) {
...
// const SidebarMenu = () => {
const SidebarMenu: React.FC<SidebarMenuProps> = ({ menuConfig }) => {
   console.log('Filtered menu:', menuConfig); 
  ...
  const { setCurrentMenuItem, getCurrentMenuItem } = useMenus();
  const location = useLocation();
  const activeMenu = getCurrentMenuItem();

  const queryClient = useQueryClient();
  const { data: roles = [] } = useQuery<RolePermission[]>({
    queryKey: ['roles'],
    queryFn: async () => {
  ...
  const auth = useContext(MenuAuthContext);
  ...
  const handleMenuClick =
    ...
    (e: MouseEvent<HTMLElement>): void => {
      // Metronic will still handle navigation based on "path"
      // We just log access here.
      setCurrentMenuItem(item);
      if (item.title || item.path) {
        void logAccess(item.title, item.path);
      }
    };
  ...
  const buildMenuItemChild = (item: IMenuItemConfig, index: number, level: number = 0) => {
    if (item.children) {
      ...
      const isActive =
        location.pathname === item.path && activeMenu?.instrument === item.instrument;

      return (
        <MenuItem key={index} className={isActive ? 'active' : ''}>
          <MenuLink
            path={item.path}
            className={clsx(
  ...
  const buildMenuHeading = (item: IMenuItemConfig, index: number) => {
    return (
      <MenuItem key={index} className="pt-2.25 pb-px">
        <MenuHeading className={clsx('flex flex-col grow', itemsGap)}>
  ...
  return (
    <Menu highlight={true} multipleExpand={false} className={clsx('flex flex-col grow', itemsGap)}>
      {/* {filteredMenu && buildMenu(filteredMenu)} */}
